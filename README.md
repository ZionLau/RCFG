...
This repository contains reproduction code and command templates for the Text8 and LM1B experiments in **Reliability-Calibrated Guidance for Few-Step Categorical Flow-Map Text Generation**.

This repository does **not** distribute flow-map checkpoints, raw CSV/JSONL outputs, generated tables, HuggingFace caches, or generated result files. The `.gitignore` excludes these files. The required flow-map checkpoints should be reproduced locally following the original Semicat training procedure before running the evaluation commands below.

## Overview

This package is designed to be copied into the root of the original Categorical Flow-Map / Semicat repository.

Base Text8/LM1B data preparation and flow-map training follow the original Semicat project. We do not redistribute the trained flow-map weights; after reproducing the base checkpoints locally, the commands in this repository run the RCFG guidance and evaluation pipeline.

Required base checkpoints:

```text
Text8: 100k-step Categorical Flow-Map checkpoint
LM1B:  500k-step Categorical Flow-Map checkpoint
```
...
semicat/
  ...
  logs/train/lm1b_500k/checkpoints/last.ckpt
  reward_models/
```
...
3. make the locally trained Text8 100k checkpoint available at `logs/train/text8_100k/checkpoints/last.ckpt`;
4. make the locally trained LM1B 500k checkpoint available at `logs/train/lm1b_500k/checkpoints/last.ckpt`;
...
```bash
git clone <SEMICAT_REPOSITORY_URL> semicat
cd semicat
```
...
## Step 1: Prepare the reproduced flow-map checkpoints

After training, make sure the Text8 100k checkpoint is available here:

```text
logs/train/text8_100k/checkpoints/last.ckpt
```

After training, make sure the LM1B 500k checkpoint is available here:
...
logs/train/lm1b_500k/checkpoints/last_uncompiled.ckpt
```
