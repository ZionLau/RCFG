...
export LM1B_RUN_DIR="${ROOT}/logs/train/lm1b_500k"
...
if [ ! -f "$LM1B_CKPT_COMPILED" ] && [ ! -f "$LM1B_CKPT" ]; then
  ...
  echo "or:"
  echo "  $LM1B_CKPT"
  echo "Please follow the original Semicat/Categorical Flow-Map procedure to train the LM1B 500k checkpoint,"
  echo "then make it available at logs/train/lm1b_500k/checkpoints/last.ckpt."
  exit 1
fi
