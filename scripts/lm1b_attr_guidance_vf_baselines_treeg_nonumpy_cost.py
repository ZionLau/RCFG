...
class GPT2PPL:
    ...
    def score(self, texts: List[str], batch_size: int = 4, max_length: int = 256) -> Dict[str, float]:
        """Evaluate generated text with GPT-2-large.

        gen_nll is the corpus/token-weighted causal-LM NLL.
        gen_ppl is the arithmetic mean of per-sequence perplexities.
        Because these use different aggregation rules, gen_ppl is not in
        general equal to exp(gen_nll).
        """
        total_loss = 0.0
        total_tok = 0
        seq_ppl_sum = 0.0
        ...
        for i in range(0, len(texts), batch_size):
            ...
            # Causal LM loss predicts token t from positions < t, so the first
            # token of each sequence is excluded by shifting labels/logits.
            ...
            # Corpus/token-weighted NLL.
            total_loss += float(token_loss.masked_select(valid).sum().item())
            total_tok += int(valid.sum().item())

            # Per-sequence perplexity, then arithmetic mean across sequences.
            seq_loss_sum = (token_loss * valid).sum(dim=1)
            seq_tok = valid.sum(dim=1)
            keep = seq_tok > 0
            if keep.any():
                seq_nll = seq_loss_sum[keep] / seq_tok[keep]
                seq_ppl = torch.exp(torch.clamp(seq_nll, max=50.0))
                seq_ppl_sum += float(seq_ppl.sum().item())
                seq_count += int(keep.sum().item())

        if total_tok == 0 or seq_count == 0:
            return {"gen_nll": float("nan"), "gen_ppl": float("nan")}

        nll = total_loss / total_tok
        ppl = seq_ppl_sum / seq_count
                        ...
                        for ml in lambdas:
                            for qlam in qlams:
                                ...
                                if args.save_samples:
                                    sample_path = str(Path(args.out).with_suffix("")) + f"_{task}_{method}_nfe{nfe}_step{step_size}_eta{eta}_mix{ml}_q{qlam}.txt"
